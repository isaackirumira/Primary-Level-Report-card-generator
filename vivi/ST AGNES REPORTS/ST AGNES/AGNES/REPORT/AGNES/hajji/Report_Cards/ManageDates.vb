...
Public Class ManageDates
    ...
    Public Sub checktoseeifdateexists()
        Try
            Dim myConnection As String = "datasource=localhost;port=3306;username=root;"
            Dim mycon As MySqlConnection = New MySqlConnection(myConnection)
            Dim SelectCommand As MySqlCommand = New MySqlCommand("Select * from reportcards.dates where year='" + txt_year.Text + "' and term='" + txt_term.Text + "';", mycon)
            Dim myReader As MySqlDataReader
            ...
            If (count = 0) Then
                sav()
            ElseIf (count = 1) Then
                MessageBox.Show("You have already set the dates unless you want to update them")
